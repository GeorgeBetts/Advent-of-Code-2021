...
foreach ($numbers as $key => $number) {
    $latestWins = $bingo->callNumber($number);
    foreach ($latestWins as $winningCard) {
        $winningCards[] = $winningCard;
    }
}

echo $winningCards[array_key_last($winningCards)]->getScore();
...
class Bingo
{
    ...
    public function callNumber($number)
    {
        ...
        foreach ($this->cards as $card) {
            // number only needs to be checked if the card has not already won
            if ($card->winningNumber == null) {
    ...
    public function checkNumber($number)
    {
        foreach ($this->grid as &$item) {
            if ($item['value'] == $number) {
                $item['marked'] = true;
                // only check the win condition if the card has not already won
                if ($this->winningNumber == null && $this->checkWin()) {
